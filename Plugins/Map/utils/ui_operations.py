...
def execute_offset_update():
    ...
    try:
        if offset_handler.update_offset_config():
            logging.info(
                "The offset configuration has been updated, and the data is being reloaded..."
            )
            return True
        else:
...
def generate_rules():
    ...
    try:
        ...
        if offset_handler.generate_rules(config):
            logging.info(
                "The rules configuration has been generated, and the data is being reloaded..."
            )
            return True
        else:
...
def use_auto_offset():
    ...
    try:
        ...
        data.plugin.state.text = (
            "Per_name data updated, rules generated. You can now use the Map plugin."
        )
        logging.warning(
            "Per_name data updated, rules generated. You can now use the Map plugin."
        )
